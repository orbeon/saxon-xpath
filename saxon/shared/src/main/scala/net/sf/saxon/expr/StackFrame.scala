...
class StackFrame {
  ...
  var map: SlotManager = _

  var slots: Array[Sequence] = _

  def this(map: SlotManager, slots: Array[Sequence]) {
    this()
    this.map = map
    this.slots = slots
  }
  ...
  def copy(): StackFrame = {
    val v2: Array[Sequence] = Arrays.copyOf(seqArray, seqArray.length)
    val s: StackFrame = new StackFrame(map, v2)

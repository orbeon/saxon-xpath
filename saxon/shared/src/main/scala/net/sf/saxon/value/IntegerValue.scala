...
object IntegerValue {
  ...
  private var ranges: Array[Long] = Array(
  ...
  def makeIntegerValue(value: Double): ConversionResult = {
    ...
    if (value > java.lang.Long.MAX_VALUE || value < java.lang.Long.MIN_VALUE) {
      if (value == Math.floor(value)) {
        new BigIntegerValue(
          FormatNumber.adjustToDecimal(value, 2).toBigInteger())
      } else {
    ...
    while (i < ranges.length) {
      if (ranges(i) == fp) {
        ...
        if (max == NO_LIMIT) {
          ...
          Int64Value.makeIntegerValue(max)
        }
      }
      i += 3
    }
    ...
    while (i < ranges.length) {
      if (ranges(i) == `type`.getFingerprint) {
        ...
        if (max == NO_LIMIT) {
          ...
          BigInteger.valueOf(max).compareTo(big) >= 0
        }
      }
      i += 3
    }
    ...
    if (last - start < 16) {
      ...
      while (i <= last) {
        ...
        if (d >= '0' && d <= '9') {
          ...
          new ValidationFailure(
            "Cannot convert string " + Err
              .wrap(s, Err.VALUE) + " to an integer")
        }
      }
      Int64Value.makeIntegerValue(if (negative) -value else value)
    } else {
      try {
        var t: CharSequence = Whitespace.trimWhitespace(s)
        if (t.charAt(0) == '+') {
  ...
  def castableAsInteger(input: CharSequence): ValidationFailure = {
    val s: CharSequence = Whitespace.trimWhitespace(input)
    val last: Int = s.length - 1
    if (last < 0) {
      new ValidationFailure("Cannot convert empty string to an integer")
    }
    var i: Int = 0

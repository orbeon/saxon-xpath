...
object AnySimpleType extends Enumeration {

  val INSTANCE: AnySimpleType = new AnySimpleType()

  class AnySimpleType extends Val with SimpleType {
...
//return;
  ...
  def getInstance(): AnySimpleType = INSTANCE

  val NAME: StructuredQName =
    new StructuredQName("xs", NamespaceConstant.SCHEMA, "anySimpleType")

  implicit def convertValue(v: Value): AnySimpleType =
    v.asInstanceOf[AnySimpleType]

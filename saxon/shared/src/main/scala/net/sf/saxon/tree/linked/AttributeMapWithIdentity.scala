...
class AttributeMapWithIdentity(private var attributes: List[AttributeInfo])
  ...
  private def isDeleted(info: AttributeInfo): Boolean =
    (info.isInstanceOf[AttributeInfo.Deleted])

  override def get(name: NodeName): AttributeInfo =
    attributes.asScala
      .find(
        info =>
          info.getNodeName == name && !(info
            .isInstanceOf[AttributeInfo.Deleted]))
      .getOrElse(null)

  override def get(uri: String, local: String): AttributeInfo = {
    for (info <- attributes.asScala) {
      val name: NodeName = info.getNodeName
      if (name.getLocalPart == local && name.hasURI(uri) && !(info
        .isInstanceOf[AttributeInfo.Deleted])) {
        info
      }
    }

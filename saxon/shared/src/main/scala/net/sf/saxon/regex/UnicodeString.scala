...
object UnicodeString {
  ...
  def makeUnicodeString(in: CharSequence): UnicodeString = {
    if (in.isInstanceOf[UnicodeString]) {
      in.asInstanceOf[UnicodeString]
    }
    if (in.length == 0) {
      EmptyString.THE_INSTANCE
    }
    val width: Int = getMaxWidth(in)
    if (width == 1) {
      new LatinString(in)
    } else if (width == 2) {
      new BMPString(in)
  ...
  def containsSurrogatePairs(value: CharSequence): Boolean =
    if (value.isInstanceOf[BMPString] || value.isInstanceOf[LatinString] ||
      ...
      false
    } else if (value.isInstanceOf[GeneralUnicodeString]) {
      val gus: GeneralUnicodeString = value.asInstanceOf[GeneralUnicodeString]

...
//import org.orbeon.saxon.{Configuration, Version}
import org.orbeon.saxon.lib.{Feature, FeatureKeys, NamespaceConstant, Validation}

/**
 * Saxon implementation of the JAXP 1.3 XPathFactory
 */
object XPathFactoryImpl {
  private val FEATURE_SECURE_PROCESSING = XMLConstants.FEATURE_SECURE_PROCESSING
}
 ...
 */
  extends XPathFactory with Configuration.ApiProvider {
  private var config: Configuration = Configuration.newConfiguration
  private var variableResolver: XPathVariableResolver = _
  private var functionResolver: XPathFunctionResolver = _
   ...
   *         { @link org.orbeon.saxon.lib.NamespaceConstant#OBJECT_MODEL_SAXON},
   *                 { @link XPathConstants#DOM_OBJECT_MODEL},
   *                 { @link org.orbeon.saxon.lib.NamespaceConstant#OBJECT_MODEL_JDOM}, or
   *                 { @link NamespaceConstant#OBJECT_MODEL_XOM}, or
   *                 { @link org.orbeon.saxon.lib.NamespaceConstant#OBJECT_MODEL_DOM4J}.
   ...
   */
  override def isObjectModelSupported(model: String) = {
    val debug = System.getProperty("jaxp.debug") != null
    val result = silentIsObjectModelSupported(model)
    if (debug) {
      System.err.println("JAXP: Calling " + getClass.getName + ".isObjectModelSupported(\"" + model + "\")")
      System.err.println("JAXP: -- returning " + (if (result) "true"
      else "false (check all required libraries are on the class path"))
    }
    result
  }

  private def silentIsObjectModelSupported(model: String) = model == NamespaceConstant.OBJECT_MODEL_SAXON || config.getExternalObjectModel(model) != null
   ...
   * <li> {@link org.orbeon.saxon.lib.FeatureKeys#SCHEMA_VALIDATION}: requests schema validation of source documents.
   ...
   */
  @throws[XPathFactoryConfigurationException]
  override def setFeature(feature: String, b: Boolean) = if (feature == XPathFactoryImpl.FEATURE_SECURE_PROCESSING) config.setBooleanProperty(Feature.ALLOW_EXTERNAL_FUNCTIONS, !b)
  else if (feature == FeatureKeys.SCHEMA_VALIDATION) config.setSchemaValidationMode(if (b) Validation.STRICT
  else Validation.STRIP)
  else try config.setBooleanProperty(feature, b)
  catch {
   ...
   * <li> {@link org.orbeon.saxon.lib.FeatureKeys#SCHEMA_VALIDATION}: requests schema validation of source documents. </li>
   * </ul>
   * <p>In addition, any Saxon configuration feature (listed in {@link FeatureKeys} can be used
   * provided the value is a boolean. (For non-boolean configuration properties, drop down to the underlying
   ...
   */
  @throws[XPathFactoryConfigurationException]
  override def getFeature(feature: String) = if (feature == XPathFactoryImpl.FEATURE_SECURE_PROCESSING) !config.getBooleanProperty(Feature.ALLOW_EXTERNAL_FUNCTIONS)
  else if (feature == FeatureKeys.SCHEMA_VALIDATION) config.getSchemaValidationMode == Validation.STRICT
  else try {
    val o = config.getConfigurationProperty(feature)
    if (o.isInstanceOf[Boolean]) o.asInstanceOf[Boolean]
    else throw new XPathFactoryConfigurationException("Configuration property " + feature + " is not a boolean (it is an instance of " + o.getClass + ")")
  } catch {
   ...
   */
  override def setXPathVariableResolver(xPathVariableResolver: XPathVariableResolver) = variableResolver = xPathVariableResolver
   ...
   */
  override def setXPathFunctionResolver(xPathFunctionResolver: XPathFunctionResolver) = functionResolver = xPathFunctionResolver
   ...
   */
  /*@NotNull*/ override def newXPath = {
    val xpath: XPathEvaluator = new XPathEvaluator(config)

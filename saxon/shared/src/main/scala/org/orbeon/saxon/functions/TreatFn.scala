...
import org.orbeon.saxon.expr.{CardinalityCheckingIterator, StaticProperty, XPathContext}
import org.orbeon.saxon.expr.parser.RoleDiagnostic
import org.orbeon.saxon.om.{LazySequence, Sequence}
...
abstract class TreatFn extends SystemFunction {
  ...
  def call(context: XPathContext, arguments: Array[Sequence]): Sequence = {
    var iterator = arguments(0).iterate()
    val card                 = getRequiredCardinality
    val role                 = makeRoleDiagnostic()

...
import org.orbeon.saxon.ma.map.{HashTrieMap, MapItem}
import org.orbeon.saxon.model.{BuiltInAtomicType, ItemType, Type}
import org.orbeon.saxon.om._
import org.orbeon.saxon.s9api.Location
import org.orbeon.saxon.style.StylesheetPackage
import org.orbeon.saxon.sxpath.{IndependentContext, XPathVariable}
...
class EvaluateInstr(xpath: Expression,
  ...
  override def operands: java.lang.Iterable[Operand] = {
    val sub = new ArrayList[Operand](8)
    if (xpathOp != null)
      sub.add(xpathOp)
  ...
  override def iterate(context: XPathContext): SequenceIterator = {
    ...
    if (expr == null) {
      val options: MapItem =
          if (optionsOp == null)
            ...
            optionsOp.getChildExpression
              .evaluateItem(context)
              .asInstanceOf[MapItem]
      val env = new IndependentContext(config) {
        override def issueWarning(s: String, locator: Location): Unit = {
          val message = "In dynamic expression {" + exprText + "}: " + s
      ...
      for (lib <- libraryList0.getLibraryList.asScala) {
        lib match {
          case builtInFunctionSet: BuiltInFunctionSet if builtInFunctionSet.getNamespace == NamespaceConstant.FN =>
      ...
      val locals = new HashMap[StructuredQName, Integer]
      if (dynamicParams != null) {
        dynamicParams.keys.forEachOrFail(paramName => {
          ...
          val `var` = env.declareVariable(paramName.asInstanceOf[QNameValue])
          locals.put(paramName.asInstanceOf[QNameValue].getStructuredQName, `var`.getLocalSlotNumber)
        })
      }
      ...
      val cit = config.makeContextItemStaticInfo(
        contextItemType,
    ...
    if (focus == null) {
      ...
      val mono = new ManualIterator(focus)
      c2.setCurrentIterator(mono)
    }
    c2.openStackFrame(slotMap)
    if (getActualParams != null) {
      for (i <- getActualParams.indices) {
        val slot = slotMap.getVariableMap.indexOf(getActualParams()(i).getVariableQName)
    ...
    if (dynamicParams != null) {
      ...
      while ({
        ...
        val slot = slotMap.getVariableMap.indexOf(paramName.getStructuredQName)
        if (slot >= 0)
          c2.setLocalVariable(slot, dynamicParams.get(paramName))
      }
    }
    for (decVal <- declaredVars.asScala) {
      val name = decVal.getVariableQName
      ...
      if (dynamicParams != null &&
        dynamicParams.get(new QNameValue(name, BuiltInAtomicType.QNAME)) == null &&
        ! isActualParam(name) &&
  ...
  def getContextItemExpr: Expression =
    if (contextItemOp == null)
      ...
      contextItemOp.getChildExpression
  ...
  def getNamespaceContextExpr: Expression =
    if (namespaceContextOp == null)
      ...
      namespaceContextOp.getChildExpression
  ...
  def getSchemaAwareExpr: Expression =
    if (schemaAwareOp == null)
      ...
      schemaAwareOp.getChildExpression
  ...
  def isActualParam(name: StructuredQName): Boolean =
    actualParams.exists(_.getVariableQName == name)

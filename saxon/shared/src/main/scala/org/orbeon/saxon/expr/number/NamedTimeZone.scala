...
// ORBEON: `java.util.TimeZone` is not supported by Scala.js so use `java.time`.
object NamedTimeZone {

  private val knownTimeZones: ju.Collection[String] = new ju.HashSet[String](50)

  private val idForCountry: ju.HashMap[String, ju.List[String]] =
    new ju.HashMap[String, ju.List[String]](50)

  private val worldTimeZones: ju.List[String] = new ju.ArrayList(20)

  ZoneId.getAvailableZoneIds.asScala.foreach(knownTimeZones.add)

  private def tz(country: String, zoneId: String): Unit = {
    var list = idForCountry.get(country)
    if (list == null)
  ...
  def getTimeZoneNameForDate(date: DateTimeValue, place: String): String = {
    ...
    val tzMinutes                      = date.getTimezoneInMinutes

    val zone =
      if (place.contains("/")) {
        getNamedTimeZone(place)
      } else {
        ...
        val epochDateMillis = date.secondsSinceEpoch.longValue * 1000

        def fromZones(zones: ju.List[String]): Option[ZoneId] =
          zones.iterator.asScala.map(ZoneId.of).find { tz =>
            tz != null && tz.getRules.getOffset(Instant.ofEpochMilli(epochDateMillis)).getTotalSeconds == tzMinutes * 60
          }

        fromZones(possibleZones)
          .orElse(fromZones(worldTimeZones))
      }

    zone.map(_.getDisplayName(TextStyle.SHORT, Locale.US)) // ORBEON: Locale.US
      .getOrElse(formatTimeZoneOffset(date))
...
//    zone
  ...
  def getOlsenTimeZoneName(date: DateTimeValue, country: String): String = {
    ...
    val possibleIds = idForCountry.get(country.toLowerCase())
...
//    val inSummerTime = exampleZone.inDaylightTime(javaDate)
//    val tzMinutes = date.getTimezoneInMinutes
//    for (i <- 0 until possibleIds.size) {
//      val olsen            = possibleIds.get(i)
//      val possibleTimeZone = TimeZone.getTimeZone(olsen)
//      val offset = possibleTimeZone.getOffset(javaDate.getTime)
//      if (offset == tzMinutes * 60000) {
//        if (inSummerTime)
//          return olsen + "*" else olsen
//      }
//    }
  ...
  def inSummerTime(date: DateTimeValue, region: String): java.lang.Boolean = {
    ...
    if (region.length == 2) {
      val possibleIds = idForCountry.get(region.toLowerCase())
      if (possibleIds == null) {
        return null
      } else {
...
//  }
  ...
  // ORBEON: 2023-11-02: only used by `FormatDate`
  def getNamedTimeZone(olsonName: String): Option[ZoneId] =
    if (knownTimeZones.contains(olsonName))
      Some(ZoneId.of(olsonName))
    else
      None
}

...
import org.orbeon.saxon.lib.{ErrorReporter, StringCollator}
import org.orbeon.saxon.s9api.OccurrenceIndicator._
import org.orbeon.saxon.sxpath.{IndependentContext, XPathEvaluator, XPathExpression, XPathVariable}
...
class XPathCompiler(@BeanProperty var processor: Processor) {
  ...
  def declareVariable(qname: QName,
    ...
    val `var`: XPathVariable =
      env.declareVariable(qname.getNamespaceURI, qname.getLocalName)
    `var`.setRequiredType(
      org.orbeon.saxon.value.SequenceType.makeSequenceType(itemType.getUnderlyingItemType,

...
abstract class AbstractVirtualNode extends VirtualNode {
  ...
  def getRealNode: AnyRef = {
    var u: AnyRef = this
    do u = u.asInstanceOf[VirtualNode].getUnderlyingNode
      while (u.isInstanceOf[VirtualNode]);
    u
  }
  ...
  def getAllNamespaces: NamespaceMap = node.getAllNamespaces

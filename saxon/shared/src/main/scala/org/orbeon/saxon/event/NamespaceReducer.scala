 ...
 */
  ...
  private def isNeeded(nsBinding: NamespaceBinding): Boolean = {
    ...
    if (pendingUndeclarations != null)
      for (p <- pendingUndeclarations.indices) {
   ...
   */
  private def addToStack(nsBinding: NamespaceBinding): Unit = {
    // expand the stack if necessary
    if (namespacesSize + 1 >= namespaces.length)
      namespaces = util.Arrays.copyOf(namespaces, namespacesSize * 2)
    namespaces({
      namespacesSize += 1
      namespacesSize - 1
    }) = nsBinding
   ...
   */
  def isDisinheritingNamespaces: Boolean = depth > 0 && disinheritStack(depth - 1)
   ...
   */
  def iteratePrefixes: util.Iterator[String] = {

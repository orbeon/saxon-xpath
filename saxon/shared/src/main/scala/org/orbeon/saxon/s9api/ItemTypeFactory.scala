...
import org.orbeon.saxon.model.{AnyExternalObjectType, AtomicType, JavaExternalObjectType, SchemaDeclaration, SchemaType, Type}
...
class ItemTypeFactory(private var processor: Processor) {
  ...
  def getDocumentTest(elementTest: ItemType): ItemType = {
    val test: org.orbeon.saxon.model.ItemType = elementTest.getUnderlyingItemType
  ...
  def exposeItemType(it: org.orbeon.saxon.model.ItemType): ItemType =

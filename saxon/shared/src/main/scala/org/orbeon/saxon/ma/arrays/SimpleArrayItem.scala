...
object SimpleArrayItem {
  ...
  def makeSimpleArrayItem(input: SequenceIterator): SimpleArrayItem = {
    val members = new ArrayList[GroundedValue]
    input.forEachOrFail(
      item =>
        if (item.getClass.getName == ("com.saxonica.functions.extfn.ArrayMemberValue"))
          members.add(
            item
    ...
    val result = new SimpleArrayItem(members)
...
class SimpleArrayItem extends AbstractArrayItem with ArrayItem {
  ...
  private var knownToBeGrounded = false
  ...
  def put(index: Int, newValue: GroundedValue): ArrayItem = {
    val a2 = new ImmutableArrayItem(this)

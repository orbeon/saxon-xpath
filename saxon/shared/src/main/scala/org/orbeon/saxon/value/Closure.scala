...
class Closure extends Sequence with ContextOriginator {
  ...
  def saveContext(expression: Expression, context: XPathContext): Unit = {
    ...
    if ((expression.getDependencies & StaticProperty.DEPENDS_ON_LOCAL_VARIABLES) != 0) {
      val localStackFrame = context.getStackFrame
      val local = localStackFrame.getStackFrameValues
// computed on first call
      val slotsUsed = expression.getSlotsUsed
      if (local != null) {
        val stackFrameMap = localStackFrame.getStackFrameMap
        val savedStackFrame = Array.ofDim[Sequence](stackFrameMap.getNumberOfVariables)
        for (i <- slotsUsed) {
          local(i) match {
            case closure: Closure =>
              val cdepth = closure.depth
              if (cdepth >= 10)
    ...
    val currentIterator = context.getCurrentIterator
    if (currentIterator != null) {
      val contextItem = currentIterator.current

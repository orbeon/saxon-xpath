...
import org.orbeon.saxon.value.DurationValue._
...
object DurationValue {
  ...
  def makeDuration(s: CharSequence,
    ...
    while (tok.hasMoreElements) {
      ...
      val delim = tok.nextElement().asInstanceOf[String].charAt(0)
      delim match {
        case 'Y' =>
          if (state > 0)
        ...
        case 'S' =>
          ...
          if (!allowDT)
            return badDuration(
              ...
              s)
          if (state == 7) {
            val frac = new StringBuilder(part)
            while (frac.length < 9)
              frac.append("0")
            part = frac.toString
            if (part.length > 9)
  ...
  def badDuration(msg: String, s: CharSequence): ValidationFailure = {
    val err = new ValidationFailure("Invalid duration value '" + s + "' (" + msg + ')')
  ...
  class DurationComparable(private var months: Int,
    ...
    def compareTo(other: DurationComparable): Int = //            }
      if (months == other.months) {
        ...
        val oneDay = 24e0 * 60e0 * 60e0
        val min0   = monthsToDaysMinimum(months) * oneDay + seconds
        val max0 = monthsToDaysMaximum(months) * oneDay + seconds
        val min1 = monthsToDaysMinimum(other.months) * oneDay + other.seconds
        val max1 = monthsToDaysMaximum(other.months) * oneDay + other.seconds
        if (max0 < min1)
          -1
        else if (min0 > max1)
          +1
    ...
    private def monthsToDaysMinimum(months: Int): Int = {
      ...
      if (months < 12) {
        ...
        val yearDays        = years * 365 + (years % 4) - (years % 100) + (years % 400) - 1
    ...
    private def monthsToDaysMaximum(months: Int): Int = {
      ...
      if (months < 12) {
        val longest = Array(0, 31, 62, 92, 123, 153, 184, 215, 245, 276, 306, 337)
        longest(months)
      } else {
        val years           = months / 12
        val remainingMonths = months % 12
...
class DurationValue extends AtomicValue with AtomicMatchKey {
  ...
  def this(years: Int,
    ...
    val h = getDays * 24L + getHours
    val m = h * 60L + getMinutes
  ...
  def signum(): Int =
    if (negative)
      -1
    else if (months == 0 && seconds == 0L && nanoseconds == 0)
      0
    else
      +1
  ...
  def getPrimitiveStringValue: CharSequence = {
    ...
    sb.append("P")
    if (years != 0)
      sb.append(years.toString + "Y")
    if (months != 0)
      sb.append(months.toString + "M")
    if (days != 0)
  ...
  def add(other: DurationValue): DurationValue = {
    val err = new XPathException("Only subtypes of xs:duration can be added")
  ...
  def subtract(other: DurationValue): DurationValue = {
    val err = new XPathException("Only subtypes of xs:duration can be subtracted")
    err.setErrorCode("XPTY0004")
    err.setIsTypeError(true)
    throw err
  }
  ...
  def multiply(factor: Double): DurationValue = {
    val err = new XPathException("Only subtypes of xs:duration can be multiplied by a number")
    err.setErrorCode("XPTY0004")
    err.setIsTypeError(true)
    throw err
  }

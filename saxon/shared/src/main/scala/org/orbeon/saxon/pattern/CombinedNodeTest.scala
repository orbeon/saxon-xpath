...
class CombinedNodeTest(private var nodetest1: NodeTest,
  ...
  def getUType: UType = {
    val u1 = nodetest1.getUType
    val u2 = nodetest2.getUType
    operator match {
  ...
  override def test(node: NodeInfo): Boolean = operator match {
    case Token.UNION =>
      nodetest1 == null || nodetest2 == null || nodetest1.test(node) || nodetest2.test(node)
    case Token.INTERSECT =>
      (nodetest1 == null || nodetest1.test(node)) && (nodetest2 == null || nodetest2.test(node))
    case Token.EXCEPT =>
      (nodetest1 == null || nodetest1.test(node)) && !(nodetest2 == null || nodetest2.test(node))
    case _ =>
      throw new IllegalArgumentException("Unknown operator in Combined Node Test")
    ...
    if (nodetest1.isInstanceOf[NameTest] && operator == Token.INTERSECT) {
      ...
      nodetest2 match {
        case contentTypeTest: ContentTypeTest =>
          var schemaType = contentTypeTest.getSchemaType
          if (forExport)
      ...
      val name = nodetest1.getMatchingNodeName.getEQName
      skind + name + content + ')'
    } else {
      val nt1 = if (nodetest1 == null) "item()" else nodetest1.toString
      val nt2 = if (nodetest2 == null) "item()" else nodetest2.toString
  ...
  def addTypeDetails(map: DictionaryMap): Unit = {
    if (nodetest1.isInstanceOf[NameTest] && operator == Token.INTERSECT) {
      ...
      nodetest2 match {
        case contentTypeTest: ContentTypeTest =>
          val schemaType = contentTypeTest.getSchemaType
          if (schemaType != Untyped.getInstance && schemaType != BuiltInAtomicType.UNTYPED_ATOMIC) {
            map.initialPut(
              "c",
              new StringValue(schemaType.getEQName + (if (nodetest2.isNillable) "?" else ""))
            )
          }
        case _ =>
      }
  ...
  override def getPrimitiveType: Int = {
    ...
    if (mask == UType.ELEMENT)
      Type.ELEMENT
    else if (mask == UType.ATTRIBUTE)
      Type.ATTRIBUTE
    else if (mask == UType.DOCUMENT)
      Type.DOCUMENT
    else
      Type.NODE
  }
  ...
  override def getContentType: SchemaType = {
    val type1 = nodetest1.getContentType
    val type2 = nodetest2.getContentType
    if (type1.isSameType(type2))
      return type1
    if (operator == Token.INTERSECT) {
      if (type2.isInstanceOf[AnyType] || ((type2 eq AnySimpleType) && type1.isSimpleType))
        return type1
      if (type1.isInstanceOf[AnyType] || ((type1 eq AnySimpleType) && type2.isSimpleType))
        return type2
    }
    AnyType.getInstance
  }
  ...
  override def getAtomizedItemType: AtomicType = {
    val type1 = nodetest1.getAtomizedItemType
    val type2 = nodetest2.getAtomizedItemType
    if (type1.isSameType(type2))
      return type1
    if (operator == Token.INTERSECT) {
  ...
  override def isAtomizable(th: TypeHierarchy): Boolean = operator match {
    case Token.UNION     => nodetest1.isAtomizable(th) || nodetest2.isAtomizable(th)
    case Token.INTERSECT => nodetest1.isAtomizable(th) && nodetest2.isAtomizable(th)
  ...
  override def getFingerprint: Int = {
    val fp1 = nodetest1.getFingerprint
    val fp2 = nodetest2.getFingerprint
    if (fp1 == fp2)
      fp1
    else if (fp2 == -1 && operator == Token.INTERSECT)
      fp1
    else if (fp1 == -1 && operator == Token.INTERSECT)
      fp2
    else
      -1
  }
  ...
  override def getMatchingNodeName: StructuredQName = {
    val n1 = nodetest1.getMatchingNodeName
    val n2 = nodetest2.getMatchingNodeName
    if (n1 != null && n1 == n2)
      n1
    else if (n1 == null && operator == Token.INTERSECT)
      n2
    else if (n2 == null && operator == Token.INTERSECT)
      n1
    else
      null
  }

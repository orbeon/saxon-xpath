...
class CardinalityCheckingIterator(
  ...
  /*@Nullable*/
  private val first    : Item = base.next()
  ...
  if (first == null) {
    ...
    if (requiredCardinality == StaticProperty.EMPTY)
      typeError("The only value allowed for the " + role.getMessage + " is an empty sequence", role.getErrorCode)
    second = base.next()
    if (second != null && !Cardinality.allowsMany(requiredCardinality)) {
      val leaders = Array(first, second)
      typeError(
        "A sequence of more than one item is not allowed as the " +
          role.getMessage +
          CardinalityChecker.depictSequenceStart(new ArrayIterator(leaders), 2),
        role.getErrorCode
      )
    }
  }
  ...
  private def typeError(message: String, errorCode: String): Nothing = {
    val e = new XPathException(message, errorCode, locator)

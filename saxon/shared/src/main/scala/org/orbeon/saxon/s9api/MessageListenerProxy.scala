...
/**
  ...
  */
class MessageListenerProxy(private var listener: MessageListener, pipe: PipelineConfiguration)
...
// also because we store several messages in a single TinyTree; and because we fail to condense the tree.
  ...
  def getMessageListener: MessageListener = listener
  ...
  def write(item: Item): Unit = {
    ...
    listener.message(new XdmNode(item.asInstanceOf[NodeInfo]), terminate, loc)
  }
}
